...
Os únicos escopos OAuth solicitados são `user-top-read` e `user-read-recently-played`. Usuários que autorizaram o SoundScope antes da inclusão de `user-read-recently-played` precisam **desconectar o Spotify, conectar novamente e autorizar o novo acesso**.
...
### Últimas escutas (Meu SoundScope V1.5)

Conforme a [referência oficial do Spotify para Recently Played](https://developer.spotify.com/documentation/web-api/reference/get-recently-played), o frontend consulta diretamente `GET https://api.spotify.com/v1/me/player/recently-played` com o scope `user-read-recently-played`. O endpoint aceita `limit` (1–50; padrão 20) e os cursores mutuamente exclusivos `after` ou `before`, em Unix timestamp de milissegundos. A resposta é uma página baseada em cursor (`href`, `limit`, `next`, `cursors` e `items`); cada item fornece `track`, `played_at` e `context`. O SoundScope solicita 20 eventos e utiliza somente `played_at`, nome, artistas, álbum, imagem e `external_urls.spotify` da faixa. Reproduções repetidas são eventos independentes e não são removidas.

Os eventos ficam em `sessionStorage` por **45 segundos**, sem access token, e o controle **Atualizar** invalida esse cache. Requisições simultâneas reutilizam uma única Promise e não há polling ou retry automático. Datas e horas são agrupadas e formatadas com `Date` e `Intl` no timezone e locale do navegador, sem presumir país ou fuso.

O trajeto dos dados pessoais é exclusivamente **Spotify → navegador do usuário**: o histórico não é enviado ao backend SoundScope, AWS, fontes externas, analytics ou logs, nem recebe persistência permanente. Erros de sessão (401), permissão (403), limite de taxa (429, exibindo `Retry-After` quando fornecido), rede e timeout permanecem isolados da busca, discografia, perfil e rankings.

Na consulta à documentação oficial vigente em 17 de setembro de 2026, Recently Played permanece documentado para Web API e não aparece entre os endpoints indisponíveis no Development Mode. Aplicam-se, contudo, as [regras atuais de quota](https://developer.spotify.com/documentation/web-api/concepts/quota-modes): apps em Development Mode têm acesso restrito aos usuários explicitamente autorizados no Dashboard (e aos limites de usuários/conta Premium definidos pelo Spotify), além do rate limit normal da Web API. Essas restrições podem mudar pela plataforma; 403 pode também indicar que uma sessão antiga ainda não concedeu o novo scope.
...
**Story Mode** é uma experiência opcional, browser-only, sobre o perfil de artista já carregado. O módulo `frontend/js/story-mode.js` não consulta APIs: ele recebe do `app.js` nome, país, ano de formação, biografia, integrantes e lançamentos já enriquecidos por TheAudioDB/MusicBrainz. Capas e links Spotify só são usados quando já existem; a consulta conservadora existente ao Spotify pode ser acionada individualmente pelo usuário, nunca em lote.

A timeline contém formação somente quando `formed_year` é um ano válido e lançamentos somente quando `first_release_date` (preferencial) ou `year` fornece uma data válida. Precisões anual, mensal e diária são preservadas sem completar mês ou dia. O filtro inicial inclui exclusivamente `primary_type=Album`; **Todos os registros** expõe os demais itens datados sem reclassificá-los. IDs de álbum/release group são preservados e registros com IDs distintos não sofrem deduplicação por título.

Integrantes ficam em um arquivo contextual separado, pois o modelo não possui datas confiáveis de entrada ou saída; `active` nunca vira evento ou data. A biografia é exibida literalmente e não origina eventos. Assim, a narrativa organiza dados, mas não infere importância, sucesso, estilo, causalidade ou acontecimentos históricos.

A experiência usa scroll vertical, `IntersectionObserver`, variações determinísticas de composição e fallback editorial local para capas. Botões nativos, foco visível, fechamento por Escape, restauração de foco e atributos ARIA dão suporte a teclado e leitores de tela; `prefers-reduced-motion` remove movimentos decorativos sem ocultar conteúdo. Em telas pequenas, a composição vira uma coluna e não exige scroll horizontal. Limitações da V1: ausência de datas históricas de integrantes, exclusão de releases sem data da timeline, cobertura variável das fontes e links Spotify apenas sob demanda após correspondência segura.
