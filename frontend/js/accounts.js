...
(() => {
  ...
  const setTab=(name)=>{tabs.forEach(b=>b.classList.toggle("is-active",b.dataset.accountTab===name));login?.classList.toggle("is-hidden",name!=="login");signup?.classList.toggle("is-hidden",name!=="signup");confirmBox?.classList.add("is-hidden");recovery?.classList.add("is-hidden");reset?.classList.add("is-hidden");if(status)status.textContent=""};
  ...
  const renderSession=(session)=>{const guest=$(".account-entry__guest"),user=$(".account-entry__user"),name=$("#account-name"),avatar=$("#account-avatar");if(!session){guest?.classList.remove("is-hidden");user?.classList.add("is-hidden");return}const claims=decodeJwt(session.idToken),display=claims.name||claims["cognito:username"]||claims.email||"Minha conta";guest?.classList.add("is-hidden");user?.classList.remove("is-hidden");if(name)name.textContent=display;if(avatar)avatar.textContent=display.split(/\s+/).slice(0,2).map(x=>x[0]).join("").toUpperCase();const mn=$("#account-menu-name"),me=$("#account-menu-email"),ma=$("#account-menu-avatar");if(mn)mn.textContent=display;if(me)me.textContent=claims.email||"";if(ma)ma.textContent=display.split(/\s+/).slice(0,2).map(x=>x[0]).join("").toUpperCase()};
  entry.addEventListener("click",()=>{let s=null;try{s=JSON.parse(sessionStorage.getItem(sessionKey)||"null")}catch(_){}if(s?.expiresAt>Date.now()){menu?.classList.toggle("is-hidden");return}open()});modal.querySelectorAll("[data-account-close]").forEach(el=>el.addEventListener("click",close));tabs.forEach(b=>b.addEventListener("click",()=>setTab(b.dataset.accountTab)));document.addEventListener("keydown",e=>{if(e.key==="Escape"&&!modal.classList.contains("is-hidden"))close()});
  ...
  $("#account-forgot")?.addEventListener("click",()=>{login?.classList.add("is-hidden");tabs.forEach(b=>b.classList.remove("is-active"));recovery?.classList.remove("is-hidden");status.textContent=""});
  recoveryForm?.addEventListener("submit",async(e)=>{e.preventDefault();if(!recoveryForm.reportValidity())return;pendingEmail=String(new FormData(recoveryForm).get("email")).trim().toLowerCase();status.textContent="Enviando código…";try{await api("ForgotPassword",{ClientId:cfg.clientId,Username:pendingEmail});recovery.classList.add("is-hidden");reset.classList.remove("is-hidden");status.textContent="Código enviado para seu e-mail."}catch(err){status.textContent=messageFor(err)}});
  resetForm?.addEventListener("submit",async(e)=>{e.preventDefault();if(!resetForm.reportValidity())return;const d=new FormData(resetForm);status.textContent="Atualizando senha…";try{await api("ConfirmForgotPassword",{ClientId:cfg.clientId,Username:pendingEmail,ConfirmationCode:String(d.get("code")).trim(),Password:String(d.get("password"))});setTab("login");login.querySelector('[name="email"]').value=pendingEmail;status.textContent="Senha atualizada. Você já pode entrar."}catch(err){status.textContent=messageFor(err)}});
  $("#account-logout")?.addEventListener("click",()=>{sessionStorage.removeItem(sessionKey);renderSession(null);menu?.classList.add("is-hidden")});
